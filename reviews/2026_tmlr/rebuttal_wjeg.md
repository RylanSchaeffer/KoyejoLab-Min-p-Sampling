...
**Scope Sec. 3 to GSM8K CoT.** We extended the evidence rather than narrowing the claim. Sec. 3 now includes GPQA, the original paper's other benchmark, with the identical sweep on 16 of the 18 models. The two Gemma 2 Instruct models produced no GPQA scores under the pinned lm-eval version and are omitted; the caption says so. The conclusion matches GSM8K CoT: at equal hyperparameter budgets, the best min-p configuration is within about one percentage point of the best other sampler on every model. The abstract now reads "comprehensively sweeping the original paper's NLP benchmarks (GSM8K CoT and GPQA)." The Sec. 3 headline and the Sec. 3.1 compute statement cover both benchmarks. Sec. 6 still notes that the sweeps cover only models released through 2024 and accuracy-style benchmarks.
