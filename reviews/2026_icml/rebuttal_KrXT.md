...
The distinction from grid search: grid search optimizes hyperparameters for one method (output: one configuration); Best-of-N diagnoses whether a claimed advantage survives equalized budgets (output: comparative curves). Like pass@k (Chen et al., 2021), the contribution is not the mechanism but its application as a diagnostic protocol.
