...
The results (Fig.~\ref{fig:gpqa_diff_of_em}; Appendix~\ref{app:sec:gpqa_scores}) match those on GSM8K CoT: once every sampler is given the same number of hyperparameter configurations, the best \texttt{min-p} configuration is within roughly one percentage point of the best other sampler on every model.
